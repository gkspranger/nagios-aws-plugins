...
parser=argparse.ArgumentParser(
	description='Check the total email complaints for SES in AWS')
parser.add_argument(
	'-H',
	metavar='HOURS',
	required=True,
...
for i in sent[:4]:
	complaints=complaints+i['Complaints']

state='UNKNOWN'
status=3

if complaints >= args.critical:
	state='CRITICAL'
	status=2
elif complaints >= args.warning:
	state='WARNING'
	status=1
elif complaints < args.warning:
	state='OK'
...
print "{0} - {1} is the total email complaints in the past {2} hour(s) for SES in AWS {3} | complaints={1};{4};{5}".format(
	state,
	complaints,
	args.hours,
	args.region,
